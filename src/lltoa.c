...
/*
    ...
    For lltoa if base is 10 and value is negative, the resulting string is preceded
 ...
 */
char * ulltoac ( unsigned long long n, char * s, unsigned int base, int uppercase)
{
    int i;
 ...
 * Based on Karnighan & Ritchie code
 */
char * lltoac ( long long n, char * s, unsigned int base, int uppercase)
{
    char* ss = s;
    if(10 == base && n < 0)
    {
        *s++ = '-';
        ulltoac((unsigned long long) -n, s, base, uppercase);
    }
    else ulltoac((unsigned long long) n, s, base, uppercase);
